 function [species,min0,everage0,realnum] = next_pop(pop_size,Group1,Group2,n,k,m)
species = zeros(size(Group1));
fitness_cost1 = zeros(size(Group1,1),1);
obj_cost = zeros(size(Group1,1),1);
[Group2,~,realnum] = decode(Group2,pop_size,n,k,m);
...
prob1 = select_prob( 2*pop_size,fitness_cost1);
for a = 1:50
   species(a,:) = family1(a,:) ;
